        ...
        if isinstance(shareholders_changes, list):
            ...
            for changes in shareholders_changes:
                ...
                if transfer_date >= act_year_enacted:
                    if len(shareholder_changes_data) < 11:
                        if changes['idType'] == 'MK':
                            shareholder_id_1 = changes['shareholderId'][0:6]
                            shareholder_id_2 = changes['shareholderId'][6:8]
                            shareholder_id_3 = changes['shareholderId'][8:]
                            shareholder_id = shareholder_id_1 + '-' + shareholder_id_2 + '-' + shareholder_id_3
                        else:
                            shareholder_id = changes['shareholderId']

                        address1 = changes['address']['address1']
                        address2 = changes['address']['address2']
                        address3 = changes['address']['address3']
                        postcode = changes['address']['postcode']
                        town = changes['address']['town']
                        state = state_mapping(changes['address']['state'])

                        if '#text' in changes['dtTransfer']:
                        ...
                        shareholder_changes_data.append({
                            ...
                            'id': shareholder_id,
                            'idType': shareholder['idType'],
                            'share_in': float(changes['shareIn']),
                            'share_out': float(changes['shareOut']),
                            'share_total': float(changes['totalShare']),
                            'address1': address1,
                            'address2': address2,
                            'address3': address3,
                            'postcode': postcode,
